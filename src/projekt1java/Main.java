...
public class Main {
    ...
    private static void printAllPaperBooks(Resource[] resources) {
        Map<String, int[]> availabilityCount = new HashMap<>();

        for (Resource resource : resources) {
            if (resource instanceof PaperBook paperBook) {

                String key = paperBook.title + " " + paperBook.author;

                availabilityCount.putIfAbsent(key, new int[]{0, 0});
                int[] counts = availabilityCount.get(key);

                if (paperBook.getStatus() == Status.AVAILABLE) {
                    counts[0]++;
                } else if (paperBook.getStatus() == Status.UNAVAILABLE) {
                    counts[1]++;
                }
            }
        }
        ...
        for (Map.Entry<String, int[]> entry : availabilityCount.entrySet()) {
            String bookInfo = entry.getKey();
            int[] counts = entry.getValue();
            System.out.println(bookInfo + " available: " + counts[0] + " unavailable: " + counts[1]);
        }
    }
}